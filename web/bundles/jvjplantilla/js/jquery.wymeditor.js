...
*/
    ...
    ALT                 : "alt",
    DIALOG_LINK         : "Link",
	DIALOG_PDF			: "PDF", // PDF EXTRA DIALOG
    DIALOG_IMAGE        : "Image",
    DIALOG_TABLE        : "Table",
	...
	CREATE_PDF			: "CreatePDF", // PDF EXTRA BUTTON
...
jQuery.fn.wymeditor = function(options) {
  ...
  options = jQuery.extend({
    ...
    logoHtml:  "<a class='wym_wymeditor_link' "
              + "href='../www.wymeditor.org/default.htm'>WYMeditor</a>",
    ...
    toolsItems: [
		...
		{'name': 'CreatePDF', 'title': 'PDF', 'css': 'wym_tools_pdf'}, // PDF EXTRA
	...
	dialogPDFSelector:	   ".wym_dialog_pdf", // PDF EXTRA DIALOG
    ...
    dialogLinkHtml:  "<body class='wym_dialog wym_dialog_link'"
               ...
               + "<input class='wym_submit button blue' type='button'"
               + " value='Insert' />"
               + "<input class='wym_cancel button' type='button'"
               + "value='Cancel' />"
               + "</div>"
               + "</form>"
               + "</body>",

	// PDF EXTRA DIALOG HTML

	dialogPDFHtml:  "<body class='wym_dialog wym_dialog_pdf'"
               + " onload='WYMeditor.INIT_DIALOG(" + WYMeditor.INDEX + ")'"
               + ">"
               + "<form>"
               + "<input type='hidden' class='wym_dialog_type' value='"
               ...
               + "</div>"
               + "<div class='row row-indent'>"
               + "<input class='wym_submit graybutton' type='button'"
               + " value='NAEO' />"
               + "<input class='wym_cancel graybutton' type='button'"
               + "value='{Cancel}' />"
               + "</div>"
               + "</form>"
               + "</body>",
    ...
    dialogImageHtml:  "<body class='wym_dialog wym_dialog_image'"
               ...
               + "' />"
               + "<div class='row'>"
               + "<div id='upload_button'><input type='button' class='bluebutton' value='Kies afbeelding' /></div>"
               + "</div>"
               + "<div class='row'>"
               + "<input type='hidden' class='wym_src' value='' />"
               + "</div>"
               + "<div class='loading'>"
               + "</div>"
               + "<div class='success'>"
               ...
               + "Alleen afbeeldingen kunnen worden toegevoegd"
               + "</div>"
               + "<div class='buttons'>"
               + "<input class='wym_submit graybutton' type='button' onClick='window.close()'"
               + " value='Toevoegen' />"
               + "<input class='wym_cancel graybutton' type='button'"
               + "value='Annuleren' />"
               + "</div>"
               + "</form>"
			   + "<script src='../www.bsdewaai.nl/js/admin/ajaxupload.js'>"
               + "</script>"
               + "<script src='../www.bsdewaai.nl/js/admin/script.js'>"
               + "</script>"
               + "</body>",
...
WYMeditor.editor.prototype.exec = function(cmd) {
  ...
  switch(cmd) {
	...
	// PDF EXTRA WYMEDITORSTUFF

	case WYMeditor.CREATE_PDF:
      var container = this.container();
      if(container || this._selected_image) this.dialog(WYMeditor.DIALOG_PDF);
    break;
...
WYMeditor.editor.prototype.dialog = function( dialogType, dialogFeatures, bodyHtml ) {
  ...
  if(wDialog) {
    ...
    switch( dialogType ) {
	  ...
	  // PDF EXTRA DIALOG
	  case(WYMeditor.DIALOG_PDF):
		sBodyHtml = this._options.dialogPDFHtml;
	  break;
...
WYMeditor.INIT_DIALOG = function(index) {
  ...
  // PDF EXTRA DIALOG
  jQuery(wym._options.dialogPDFSelector + " "
    + wym._options.submitSelector).click(function() {
      ...
      if(sUrl.length > 0) {
        ...
        if (selected[0] && selected[0].tagName.toLowerCase() == WYMeditor.A) {
            ...
            wym._exec(WYMeditor.CREATE_PDF, sStamp);
            link = jQuery("a[href=" + sStamp + "]", wym._doc.body);
        }
...
*  [["src", "../www.editam.com/@a=b&c=d&f=g"], ["title", "Editam, <Simplified> CMS"]]
* or an object like {src:"../www.editam.com/@a=b&c=d&f=g", title:"Editam, <Simplified> CMS"}
...
*  [["src", "../www.editam.com/"], ["title", "Editam, Simplified CMS"]]
*/
 ...
 *        Jean-Francois Hovinne (jf.hovinne a-t wymeditor dotorg)
 *        Bermi Ferrer (wymeditor a-t bermi dotorg)
 *        FrÃƒÂ©dÃƒÂ©ric Palluel-Lafleur (fpalluel a-t gmail dotcom)
 *        Jonatan Lundin (jonatan.lundin a-t gmail dotcom)
 */
