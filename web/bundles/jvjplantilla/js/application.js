$('document').ready(function() {
    ...
    Crud inicio
     ******************/  
    $('#accion_nuevo').click(carganuevo);
	$(".cargando_contenedor").hide()
...
{
    var x=$("#ventana_derecha");
    x.html('')
    cargando_con('cargando dialogo...');
}
...
function problemas()
{
    $("#ventana_derecha").text('Problemas en el servidor.');
}
...
function LoadJS(nomarch) {
    ...
    if (ele == undefined) {
        ...
        tagjs.setAttribute("src", "/bundles/jvjplantilla/js/"+nomarch+".js?rnd="+d.getTime());
...
function cambiatipo()
{
    var val = $('#inventario_adminbundle_bientype_clase').val();
    var div=$("#bien_tipo");
	lugar='tipo/'+val;
...
function cambiadetallesparte()
{
    var val = $('#inventario_adminbundle_partestype_partesbien').val();
    var div=$("#cambiaso");
	var lugar;
	lugar='detallepartes/'+val;
	
    cargadiv(lugar,div);
}
function cambiadetallesbien()
{
    var val = $('#inventario_adminbundle_bientype_clase').val();
    var div=$("#bien_detalles");
	var lugar;
	lugar='tipodetalle/'+val;
	
    cargadiv(lugar,div);
}
function cargadiv(lugar,div){
    $.ajax({
        async:true,
        type: "get",
        dataType: "html",
        ...
        beforeSend:function (){
            div.html('');
            cargando_con('cargando detalles...');
        },
        success:function(datos){
cargando_con('detalles cambiados...');
            div.html(datos);   
            $('select').chosen();
        },
        timeout:10000,
        error:function(){
            cargando_con('Problemas en el servidor. actualise la pagina ');
        }
    }); 
    return false;
}

function cargando_con(mensaje)
{
    var con=$(".cargando_contenedor");
	con.show(1);
	con.text(mensaje);
	setTimeout(function(){ con.slideUp(100); },4000);
